...
$(document).ready(() => {
    $('#descripcion').append('<div class="col-md-6 p-lg-5 mx-auto my-5"><h4 class="fw-normal">TEST SEPSIS METODO: SVM</h4></div>')
    $('#muestra_datos').hide()
    $('#muestra_datos').append(`<h5 class="mb-3" id ="tit"></h5>
                                ...
                                `);
    let url = '/salidaSVMSup/';
    $.getJSON( url, function(data){
        $('#tit').append(`Resultados (SUPERVIVENCIA) - Kernel: Sigmoide`)
        for (let i = 0; i < data.length; i++) {
            ...
            `)
            dataTest.push(parseFloat(data[i].dataTest))
    ...
    $.getJSON( url, function(data){
        ...
        $('#tit1').append(`Resultados (VASOPRESORES) - Kernel: Sigmoide`)
        for (let i = 0; i < data.length; i++) {
